...
struct mm_solver{
    ...
    void init(int i_length, int i_num){
        length = i_length;
        num = i_num;
        if(log(pow(num, length)) > log(1953126)){
          high_order = true;
        }
...
bool check_contain(int n, const std::vector<int>& v){//return true if the vector contains n
  ...
  void pool_generator(int length, int num, int count, std::vector<int>& v){//translates a decimal number into a vector of int of length which contains the numbers of the n-numeral system number converted from the decimal integer
  ...
  void give_feedback_trial(const std::vector<int>& attempt, const std::vector<int>& trial, int num, int& black_hits, int& white_hits){//takes input of two vector sequence and compare them to return black_hits and white_hits
...
int number_of_num(const std::vector<int>& v, int num){//returns the number of the input number "num" is present in the input vector
  int n = 0;
